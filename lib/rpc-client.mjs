		...
		return new Promise((resolve, reject) => {
			...
			this.proc.on("close", (code, signal) => {
				...
				if (this._buffer.length > 0) {
					const line = this._buffer.endsWith("\r")
						? this._buffer.slice(0, -1)
						: this._buffer;
					if (line.length > 0) this._dispatch(line);
					this._buffer = "";
				}
				...
				for (const { reject } of this._pending.values()) {
					reject(
						new Error(
							`process exited (code=${code}, signal=${signal}) before response`,
						),
					);
				}
				this._pending.clear();
		...
		if (parsed.type === "response" && parsed.id != null) {
			...
			if (pending) {
				...
				if (parsed.success === false) {
					pending.reject(
						new Error(parsed.error || `command ${parsed.command} failed`),
					);
				} else {
