...
async function queryAvailableModels() {
	...
	return new Promise((resolve) => {
		const proc = spawn("pi", ["--list-models"], { stdio: ["ignore", "pipe", "pipe"] });
		let out = "";
...
async function runListModels({ args, presets, allPeers }) {
	...
	if (allPeers.length === 0) {
		...
		const modelW = Math.max(...allPeers.map((p) => (p.model ?? "(default)").length), 5);
		for (const p of allPeers) {
			...
			console.log(`  ${valid} ${p.name.padEnd(nameW)}  ${model.padEnd(modelW)}${note}`);
		}
	}
	...
	if (args.peers || args.preset) {
		...
		console.log(`  Source: ${args.preset ? `preset "${args.preset}"` : `--peers ${args.peers.join(",")}`}`);
		...
		const modelW = Math.max(...selected.map((p) => (p.model ?? "(default)").length), 5);
		for (const p of selected) {
			...
			console.log(`  ${valid} ${p.name.padEnd(nameW)}  ${model.padEnd(modelW)}${note}${override}`);
		}
	}
...
function printDryRun({ topic, peers, maxRounds, cwd, savePath }) {
	...
	console.log(`Cwd:          ${cwd}`);
	console.log(`Save path:    ${savePath ?? "(none — add --save to write a transcript)"}`);
	console.log(`Turn order:   ${peers.map((p) => `${p.name} (${p.role})`).join(" → ")}`);
	console.log("\nPeers:");
 ...
 * `consensus` is true.
 */
function renderMarkdown({ topic, peers, rounds, consensus, transcript, startedAt, endedAt, tags = [] }) {
	...
	lines.push("");
	lines.push(`> ${transcript.length} turns · ${consensus ? "consensus reached" : "max rounds reached"} after ${rounds} round(s)`);
	lines.push("");
	...
	if (consensus && transcript.length > 0) {
		...
		lines.push("");
		lines.push(`Reached at round ${last.round} by **${last.peer}** (${last.role}). The final statement above is the agreed conclusion.`);
		lines.push("");
	}
...
function parseArgs(argv) {
	const args = {
		topic: null, peers: null, preset: null, maxRounds: 12,
		listPresets: false, listModels: false, listTranscripts: false,
		show: null, latest: false, transcriptsDir: null,
		search: null, inField: null,
		tag: [], help: false, save: null, model: null, dryRun: false, validateModels: true,
	};
	for (let i = 2; i < argv.length; i++) {
		const a = argv[i];
		if (a === "--topic" || a === "-t") args.topic = argv[++i];
		else if (a === "--peers" || a === "-p") args.peers = argv[++i].split(",").map((s) => s.trim());
		else if (a === "--preset") args.preset = argv[++i];
		else if (a === "--max-rounds" || a === "-r") args.maxRounds = parseInt(argv[++i], 10);
		else if (a === "--save" || a === "-o") args.save = argv[++i];
...
async function main() {
	...
	if (args.listTranscripts) {
		const dir = args.transcriptsDir
			? (path.isAbsolute(args.transcriptsDir) ? args.transcriptsDir : path.join(process.cwd(), args.transcriptsDir))
			: process.cwd();
	...
	if (args.show !== null || args.latest) {
		await runShow({ path: args.show, latest: args.latest, transcriptsDir: args.transcriptsDir });
		return;
	}
	...
	if (!args.topic) {
		console.error("Error: --topic is required (or pass the topic as the first positional argument).");
	...
	if (args.model) {
		...
		for (const name of overrides.keys()) {
			if (!selectedNames.has(name)) {
				console.error(`Warning: --model override "${name}" doesn't match any selected peer. Ignored.`);
			}
		}
	}
	...
	if (args.save !== null) {
		if (!args.save) {
			...
			savePath = path.join(process.cwd(), `roundtable-${slugify(args.topic)}-${stamp}.md`);
	...
	if (args.validateModels) {
		const available = await queryAvailableModels();
		if (available === null) {
			console.error("⚠ Warning: could not query `pi --list-models` — skipping model validation.");
			...
			if (unknown.length > 0) {
				console.error("");
				console.error("✗ Model validation failed — aborting before spawning agents.");
				console.error("");
...
async function readTranscript(filePath) {
	...
	for (const line of m[1].split("\n")) {
		if (line.startsWith("peers:")) { mode = "peers"; continue; }
		if (line.startsWith("tags:")) { mode = "tags"; continue; }
...
async function findTranscripts(dir) {
	...
	for (const e of entries) {
		...
		try {
			...
			if (t) out.push({ path: full, mtimeMs: stat.mtimeMs, meta: t.meta, body: t.body });
...
function renderTranscript({ meta, body }, { color = true } = {}) {
	...
	if (meta.date) lines.push(c("user", `Date:     ${meta.date}${meta.ended ? ` → ${meta.ended}` : ""}`));
	...
	if (meta.outcome) {
		...
		lines.push(c(ok ? "user" : "critic", `Outcome:  ${meta.outcome}${meta.rounds == null ? "" : ` (after ${meta.rounds} round${meta.rounds === 1 ? "" : "s"})`}`));
	}
	if (meta.peers && meta.peers.length > 0) {
		lines.push(c("user", `Peers:    ${meta.peers.map((p) => `${p.name} (${p.role ?? "?"})`).join(", ")}`));
...
async function runListTranscripts(dir) {
	...
	const roundsW = 6;
	const outcomeW = 18;
	const tagsW = Math.max(...transcripts.map((t) => (t.meta.tags ?? []).join(",").length || 0), 4);
	const pathW = Math.max(...transcripts.map((t) => path.relative(dir, t.path).length), 30);
	console.log(
		`  ${"DATE".padEnd(dateW)} ${"DUR".padEnd(durW)} ${"ROUNDS".padEnd(roundsW)} ${"OUTCOME".padEnd(outcomeW)} ${"TAGS".padEnd(tagsW)} PATH`,
	);
	console.log(`  ${"-".repeat(dateW)} ${"-".repeat(durW)} ${"-".repeat(roundsW)} ${"-".repeat(outcomeW)} ${"-".repeat(tagsW)} ${"-".repeat(pathW)}`);
	for (const t of transcripts) {
		const date = new Date(t.mtimeMs).toISOString().slice(0, 19).replace("T", " ");
		const dur = t.meta.duration_seconds == null
			? "?"
			: (t.meta.duration_seconds >= 60 ? `${Math.floor(t.meta.duration_seconds / 60)}m${t.meta.duration_seconds % 60}s` : `${t.meta.duration_seconds}s`);
		...
		console.log(`  ${date.padEnd(dateW)} ${dur.padEnd(durW)} ${rounds.padEnd(roundsW)} ${outcome.padEnd(outcomeW)} ${tags.padEnd(tagsW)} ${relPath}`);
	}
}
...
async function runShow({ path: showPath, latest, transcriptsDir }) {
	...
	if (showPath && showPath !== true) {
		// Explicit path
		if (path.isAbsolute(showPath)) filePath = showPath; else filePath = path.join(process.cwd(), showPath);
	} else if (latest) {
		// Find newest in transcriptsDir (default: cwd)
		const dir = transcriptsDir ?? process.cwd();
		const transcripts = await findTranscripts(dir);
		if (transcripts.length === 0) {
			console.error(`No transcripts found in ${dir} (looking for roundtable-*.md)`);
			process.exit(1);
		}
		...
		console.error("  pi-roundtable --show --latest --transcripts-dir ~/roundtables");
		process.exit(1);
	}
	...
	if (!transcript) {
		console.error(`${filePath} doesn't look like a roundtable transcript (no frontmatter or missing topic).`);
		process.exit(1);
	}
...
async function runSearch({ dir, term, inField, tagsFilter, showAll }) {
	...
	for (const t of transcripts) {
		...
		if (inField) {
			...
			if (field === "topic") {
				...
				haystack = (t.meta.topic ?? "").toLowerCase();
			} else if (field === "outcome") {
				perLine.push({ lineNo: 1, text: t.meta.outcome ?? "" });
				haystack = (t.meta.outcome ?? "").toLowerCase();
			} else if (field === "peers" || field === "models") {
				const items = field === "peers"
					? (t.meta.peers ?? []).map((p) => `${p.name} (${p.role ?? "?"})`).join(", ")
					: (t.meta.peers ?? []).map((p) => p.model ?? "").join(", ");
				perLine.push({ lineNo: 1, text: items });
				haystack = items.toLowerCase();
			} else if (field === "tags") {
				const items = (t.meta.tags ?? []).join(", ");
				perLine.push({ lineNo: 1, text: items });
				haystack = items.toLowerCase();
			} else {
				console.error(`Unknown --in field: ${field}. Valid: topic, outcome, peers, models, tags`);
				process.exit(1);
			}
		} else {
			...
			haystack = perLine.map((l) => l.text).join("\n").toLowerCase();
		}

		const lineHits = perLine.filter((l) => l.text.toLowerCase().includes(lowerTerm));
		if (lineHits.length === 0) continue;
		matches.push({ transcript: t, lineHits });
	}

	if (matches.length === 0) {
		console.log(`No matches for "${term}"${tagsFilter.length > 0 ? ` (tags: ${tagsFilter.join(", ")})` : ""} in ${dir}`);
		return;
	}

	console.log(`Found ${matches.length} transcript${matches.length === 1 ? "" : "s"} matching "${term}"${tagsFilter.length > 0 ? ` (tags: ${tagsFilter.join(", ")})` : ""}:\n`);
	for (const m of matches) {
		...
		for (const hit of toShow) {
			const trimmed = hit.text.length > 100 ? hit.text.slice(0, 100) + "..." : hit.text;
			console.log(`    L${hit.lineNo}: ${trimmed}`);
		}
		if (!showAll && m.lineHits.length > 2) {
			console.log(`    ... +${m.lineHits.length - 2} more line${m.lineHits.length - 2 === 1 ? "" : "s"} (use --all to show all)`);
		}
		console.log("");
	}
}
