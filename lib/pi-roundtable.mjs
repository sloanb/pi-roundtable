...
// Frontmatter keys that hold YAML block lists (`key:\n  - item`), as opposed
// to the default `key: value` scalars.
const FRONTMATTER_LIST_KEYS = new Set(["capabilities"]);

export function parseFrontmatter(content) {
	const m = content.match(/^---\n([\s\S]*?)\n---\n([\s\S]*)$/);
	if (!m) return { body: content };
	const fm = {};
	let listKey = null; // key currently collecting `- item` entries
	for (const line of m[1].split("\n")) {
		// List item under the current list key (e.g. capabilities:)
		const item = line.match(/^\s+-\s+(.+)$/);
		if (item && listKey) {
			fm[listKey].push(item[1].replace(/^["']|["']$/g, "").trim());
			continue;
		}
		listKey = null; // any non-item line ends the current list
		...
		const value = kv[2]
			...
			.trim();
		if (value === "") {
			// Bare key: either starts a block list, or (for tools) explicitly
			// declares an empty list. Other scalar keys with empty values are
			// left unset so downstream defaults still apply.
			if (FRONTMATTER_LIST_KEYS.has(key)) {
				fm[key] = [];
				listKey = key;
			} else if (key === "tools") {
				fm.tools = [];
		...
		if (key === "tools") {
			// tools is an inline comma-separated list, not a block list
			fm.tools = value
				.split(",")
				.map((s) => s.trim())
				.filter(Boolean);
 ...
 * quoting mistakes (e.g. a trailing space after a line-continuation
 * backslash) that would otherwise run a whole roundtable on an empty topic.
 */
export function isBlankTopic(s) {
	return !s || s.trim() === "";
}
 ...
 */
export function checkModel(configured, available) {
	...
	return availableSet.has(bareId) ? "ok" : "unknown";
}
...
async function runListModels({ args, presets, allPeers }) {
	...
	if (args.peers || args.preset) {
		...
		const cliOverrides = parseToolsOverrides(args.tools);
		for (const peer of selected) {
 ...
 */
export function printDryRun({ topic, peers, maxRounds, cwd, savePath }) {
 ...
 * and per-turn sections. On consensus, a full `## Conclusion` section
 * (summary + details) is appended so the outcome is easy to find.
 */
export function renderMarkdown({
	...
	if (consensus && transcript.length > 0) {
		...
		const byPeer = c?.byPeer ?? last.peer;
		const byRole = c?.byRole ?? last.role;
		const byRound = c?.round ?? last.round;

		lines.push("## Conclusion");
		lines.push("");
		lines.push(
			`✅ Consensus reached at round ${byRound} by **${byPeer}** (${byRole}).`,
		);
		lines.push("");

		// The full summary — the orchestrator's summary, the concluding peer's
		// final statement, or (fallback) the last turn text.
		const summary =
			(c?.summary ?? last.text ?? "").trim() || "(no summary provided)";
		...
		// Structured artifacts rendered as human-readable Markdown
		if (c?.artifactDocs?.length) {
			for (const doc of c.artifactDocs) {
				lines.push(`#### Artifact: ${doc.name}`);
				lines.push("");
		...
		// Final findings from each peer (orchestrated mode). Rendered BEFORE
		// the summary so the orchestrator's conclusion is always the last
		// section of the file.
		if (c?.peerFindings?.length) {
			lines.push("### Peer reports");
			lines.push("");
			for (const f of c.peerFindings) {
				lines.push(`#### ${f.peer} (${f.role}) — ${f.status}`);
				lines.push("");
				lines.push((f.findings ?? "").trim() || "(no findings reported)");
				lines.push("");
			}
		}
...
// Known flags: single source of truth for "did you mean" suggestions and the
// registry-sync test. Must stay in sync with the branches below — the test
// parses every flag here and asserts no unknown-option error.
...
// Flags that consume the next token as a value (used by the registry-sync
// test to parse each flag with a value attached).
...
// Constrained flag values, validated at parse time with suggestions.
const MODES = ["auto", "sequential", "orchestrated"];
const CHANNELS = ["stable", "prerelease"];
const IN_FIELDS = ["topic", "outcome", "peers", "models", "tags"];

export function parseArgs(argv) {
	// Tri-state note: maxRounds/pretty/compact/timing/thinking/validateModels/
	// mode start as null ("not set on the CLI") so global config (config.json)
	// can supply them without shadowing explicit flags. resolveArgs() in
	// config.mjs applies config → built-in defaults to produce final values.
	const args = {
		topic: null,
		peers: null,
		preset: null,
	...
	for (let i = 2; i < argv.length; i++) {
		const a = argv[i];
		if (endOfOptions) {
			// After "--": everything is positional (topic, then extras).
			if (args.topic)
				args.warnings.push(
					`⚠ Ignoring extra positional '${a}' — only the first positional is used as the topic`,
				);
			else args.topic = a;
			continue;
		}
		...
		if (a === "--topic" || a === "-t") args.topic = argv[++i];
			...
			args.channel = argv[++i];
			if (!args.error && !CHANNELS.includes(args.channel)) {
				const s = suggestCandidates(args.channel, CHANNELS);
				args.error = `Invalid value for --channel: '${args.channel}'${s.length > 0 ? ` — did you mean ${formatSuggestions(s)}?` : ""}. Valid: ${CHANNELS.join(", ")}`;
				break;
			}
			...
			if (!args.error && !MODES.includes(args.mode)) {
				const s = suggestCandidates(args.mode, MODES);
				args.error = `Invalid value for --mode: '${args.mode}'${s.length > 0 ? ` — did you mean ${formatSuggestions(s)}?` : ""}. Valid: ${MODES.join(", ")}`;
				break;
			}
			...
			args.inField = argv[++i];
			if (!args.error && !IN_FIELDS.includes(args.inField)) {
				const s = suggestCandidates(args.inField, IN_FIELDS);
				args.error = `Invalid value for --in: '${args.inField}'${s.length > 0 ? ` — did you mean ${formatSuggestions(s)}?` : ""}. Valid: ${IN_FIELDS.join(", ")}`;
				break;
			}
		} else if (a === "--help" || a === "-h") args.help = true;
		else if (a.startsWith("-")) {
			// Flag-shaped tokens are never valid topics: a typo'd flag must error
			// with a suggestion, not silently launch a roundtable about "-show".
			const s = suggestFlag(a, KNOWN_FLAGS);
			args.error =
				s.length > 0
					? `Unknown option '${a}' — did you mean ${formatSuggestions(s)}? Run 'pi-roundtable --help' for usage.`
					: `Unknown option '${a}'. Run 'pi-roundtable --help' for usage.`;
			break;
		} else if (args.topic) {
			args.warnings.push(
				`⚠ Ignoring extra positional '${a}' — only the first positional is used as the topic`,
...
Options:
  ...
  -T, --list-transcripts [DIR]  List saved transcripts (newest first). DIR defaults
      ...
      --show [PATH|#N|--latest]  Render a saved transcript to the terminal, with
	                          color. @N opens result N of the most recent --search
                          ("#N" also works when quoted in a shell).
      ...
      --search TERM       Search all transcripts in --transcripts-dir (or cwd) for
                          ...
                          by default. Use --in to scope to a single field. Results
                          are numbered; jump to one with: --show @N
  ...
  -y, --yes               Non-interactive mode (assume yes to prompts)
      ...
      --config              Show the effective global configuration (from
                            ~/.pi-roundtable/config.json) with per-key
                            provenance, then exit
  -h, --help              Show this help

Configuration:
  Global defaults (preset, transcripts_dir, save, max_rounds, mode, channel,
  tags, model, tools, display flags) and preset aliases live in
  ~/.pi-roundtable/config.json. Run with --config to see the effective values.
  Every flag above can be defaulted there; flags always win over config.
 ...
 * peer name -> tools array. Shared by the real run (resolveTools) and the
 * --list-models display so both resolve identically.
 */
export function parseToolsOverrides(spec) {
	const overrides = new Map();
	...
	for (const part of parts) {
		...
		if (eqIdx > 0) {
			// New name=tools pair
			if (currentName) overrides.set(currentName, currentTools);
...
export function resolveTools({ selected, args, presets }) {
	const presetTools =
		args.preset && presets[args.preset]?.tools ? presets[args.preset].tools : {};
	...
	// Return resolved tools map
	const resolved = {};
	for (const peer of selected) {
		resolved[peer.name] = peer.tools;
	}
	return resolved;
}

/**
 * Resolve a transcripts-dir argument to an absolute path: absolute paths
 * pass through, relative paths resolve against cwd, null → cwd.
 */
function resolveTranscriptsDir(dir) {
...
async function main() {
	...
	// Unknown/mistyped options and stray positionals: fail fast with a
	// suggestion instead of silently launching something unintended.
	if (args.error) {
	...
	// Load global config (~/.pi-roundtable/config.json) early: it supplies
	// defaults for nearly every flag and presets aliases. A malformed config
	// is fatal on every command — the user's configured intent must not
	// silently no-op.
	const {
		config,
		path: configPath,
	...
	if (args.config) {
		console.log(renderConfigView({ args, config, path: configPath, existed }));
		return;
	}

	// Apply config defaults (CLI flag > config > built-in default).
	args = resolveArgs(args, config);

	const presets = await loadPresets();
	const allPeers = await loadPeers();

	// Resolve preset aliases ONCE, right after config resolution, so every
	// downstream consumer (--list-models, dry-run, the run itself) sees the
	// canonical preset name. args.preset may come from --preset or from
	// config defaults.preset.
	if (args.preset) {
		try {
			args.preset = resolvePresetName({
				name: args.preset,
				presets,
				config,
			});
	...
	if (args.listPresets) {
		...
		if (aliasEntries.length > 0) {
			console.log("\nAliases (from config.json):");
			for (const [alias, target] of aliasEntries) {
				console.log(`  ${alias.padEnd(20)} → ${target}`);
			}
		}
		return;
	}
	...
	if (args.listTranscripts) {
		const dir = resolveTranscriptsDir(args.transcriptsDir);
	...
	if (isBlankTopic(args.topic)) {
		if (args.topic) {
			// A whitespace-only topic is almost always a shell quoting mistake
			// (e.g. a trailing space after a line-continuation backslash).
			console.error(
				"Error: --topic is empty or only whitespace — refusing to run a roundtable with no topic.",
			);
			console.error(
				"  (Tip: check for a trailing space after a line-continuation backslash: use \\ at end of line, not \\ )",
			);
		} else {
			console.error(
	...
	let selected;
	if (args.preset) {
		// args.preset was alias-resolved earlier; this lookup can only fail if
		// the preset table changed since (defensive).
		const preset = presets[args.preset];
		if (!preset) {
			console.error(
				`Unknown preset: ${args.preset}. Available: ${Object.keys(presets).join(", ")}`,
	...
	// Auto-named saves go to the effective transcripts dir (CLI
	// --transcripts-dir > config transcripts_dir > cwd).
	let savePath = null;
	if (args.save !== null) {
		if (!args.save) {
			const stamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
			const transcriptsDir = resolveTranscriptsDir(args.transcriptsDir);
			savePath = path.join(
				transcriptsDir,
	...
	// Effective mode: explicit --mode > config defaults.mode > "auto"
	// (auto-detect orchestrated when an orchestrator peer is selected).
	if (args.mode === "orchestrated" && !hasOrchestrator) {
		console.error("\n✗ orchestrated mode requires an orchestrator peer.");
		console.error("  The selected preset does not include an orchestrator peer.");
		console.error(
		...
		console.error(
			`  (mode was set by: ${args.modeExplicit ? "--mode flag" : 'config.json "defaults.mode"'})`,
		);
		process.exit(1);
	}
	let mode = args.mode;
	...
	if (savePath !== null) {
		...
		// Ensure the transcripts dir exists (auto-saves may target a
		// configured dir that hasn't been created yet).
 ...
 */
export function renderTranscript({ meta, body }, { color = true } = {}) {
	...
	// Render the conclusion section (## Conclusion, or legacy ## Consensus) last,
	// so the outcome is the final thing shown.
	const conclusionMatch = body.match(/^## (Conclusion|Consensus)\s*$/m);
	if (conclusionMatch) {
		const conclusionBody = body
			.slice(conclusionMatch.index + conclusionMatch[0].length)
			.trim();
		if (conclusionBody) {
			lines.push("");
			lines.push(c("user", `━━━ CONCLUSION ━━━`));
			lines.push("");
			lines.push(conclusionBody);
			lines.push("");
		}
	}
	return lines.join("\n");
...
export async function runListTranscripts(dir) {
...
export async function runShow({
	...
	if (
		...
		(showPath.startsWith("@") || showPath.startsWith("#"))
	) {
		// Result reference from the last search: resolve @N (or quoted #N)
		// against the search snapshot, with a confirmation line so staleness
		// is visible.
		if (!installDir) {
			console.error("✗ Result references (@N) are not available here.");
			process.exit(1);
		}
		const resolved = await resolveResultRef({ ref: showPath, installDir });
		if (!resolved.ok) {
			console.error(`✗ ${resolved.error}`);
			process.exit(1);
		}
		const { index, entry, number } = resolved;
		const topicPreview =
			entry.topic.length > 80 ? `${entry.topic.slice(0, 77)}...` : entry.topic;
		console.log(
			`Opening result ${number} of ${index.results.length} from search "${index.term}" (${agoLabel(index.timestamp)}):`,
		);
		console.log(
			`  ${path.basename(entry.path)}${topicPreview ? ` — ${topicPreview}` : ""}`,
		);
		filePath = entry.path;
	} else if (showPath && showPath !== true) {
		// Explicit path
		...
		console.error(
			"Error: --show requires a path, an @N result reference, or --latest",
		);
		console.error("  pi-roundtable --show /path/to/transcript.md");
		console.error(
			"  pi-roundtable --show @2                (result 2 of the last --search)",
		);
	...
	console.log(renderTranscript(transcript, { color: process.stdout.isTTY }));
...
export async function runSearch({
	...
	// Snapshot writer: when installDir is given, record this result list so a
	// later `--show @N` can resolve result numbers across invocations.
	const snapshot = (results) =>
		installDir
			? writeSearchIndex({ installDir, dir, term, results })
			: Promise.resolve();
	...
	await snapshot(
		matches.map((m) => ({
			path: m.transcript.path,
			topic: m.transcript.meta.topic ?? "",
		})),
	);

	console.log(
		`Found ${matches.length} transcript${matches.length === 1 ? "" : "s"} matching "${term}"${tagsFilter.length > 0 ? ` (tags: ${tagsFilter.join(", ")})` : ""} — jump with: pi-roundtable --show @N\n`,
	);
	for (const [i, m] of matches.entries()) {
		const relPath = path.relative(dir, m.transcript.path);
		const tags = m.transcript.meta.tags ?? [];
		const tagStr = tags.length > 0 ? ` [${tags.join(", ")}]` : "";
		console.log(`━━━ [${i + 1}] ${relPath}${tagStr}`);
...
export async function handleUpdate({ args, installDir }) {
	const { spawn } = await import("node:child_process");
...
// Run the CLI only when this module is executed as a script
// (node lib/pi-roundtable.mjs ...), not when imported (tests, programmatic use).
const isMainModule =
	process.argv[1] &&
