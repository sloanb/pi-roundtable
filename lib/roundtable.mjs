...
export class Roundtable {
	/**
	 * @param {object} opts
	 * @param {Array<{name:string, role:string, model?:string, systemPrompt?:string, appendSystemPromptPath?:string}>} opts.peers
	 ...
	 * @param {string[]} [opts.tools] - tools to enable on all peers (default: read, bash, edit, write)
	...
	constructor(opts) {
		...
		this.transcript = []; // [{peer, role, text, round}]
		this.round = 0;
	}
	...
	async start() {
		...
		for (const peer of this.peers) {
			const args = [
				"--mode", "rpc",
				"--no-session",
				"--provider", peer.model?.split("/")[0] ?? "ollama-cloud",
				"--model", peer.model ?? "ollama-cloud/glm-5.3-flash",
	...
	_wireLog(client, peer) {
		client.onEvent((evt) => {
			...
			const wantStream = process.stdout.isTTY && process.env.PI_ROUNDTABLE_QUIET !== "1";
	...
	async _speak(peer) {
		...
		const abortPromise = this.signal
			? new Promise((_, reject) => this.signal.addEventListener("abort", () => reject(new Error("aborted")), { once: true }))
			: new Promise(() => {});
	...
	async run() {
		...
		this._log(this._color("user", `Peers: ${this.peers.map(p => `${p.name} (${p.role})`).join(", ")}\nRounds: ${this.maxRounds}\n\n`));
		...
		try {
			while (this.round < this.maxRounds && !consensus) {
				...
				this.transcript.push({ peer: peer.name, role: peer.role, text: result.text, round: this.round });
				if (!peer._streamedTurn) {
