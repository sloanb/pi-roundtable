...
import { isStructuredArtifact, artifactToMarkdown } from "./artifacts.mjs";
...
/**
 * Find the span of the JSON object that parseAgentJSON would extract
 * (fenced or bare), so control tokens inside it can be treated as data
 * rather than signals. Returns { start, end } or null.
 */
function findJSONSpan(text) {
	const fence = text.match(/```(?:json)?\s*\{[\s\S]*?\}\s*```/);
	if (fence) return { start: fence.index, end: fence.index + fence[0].length };
	const bare = text.match(/\{[\s\S]*\}/);
	if (bare) return { start: bare.index, end: bare.index + bare[0].length };
	return null;
}

/**
 * Remove every occurrence of `token` OUTSIDE the [start, end) span, leaving
 * the span's contents untouched. A null span strips everywhere.
 */
function stripOutsideSpan(text, span, token) {
	if (!span) return text.split(token).join("");
	return (
		text.slice(0, span.start).split(token).join("") +
		text.slice(span.start, span.end) +
		text.slice(span.end).split(token).join("")
	);
}
...
export class Roundtable {
	...
	constructor(opts) {
		...
		this._completionSignals = []; // worker [DONE]s (orchestrated): signals for the orchestrator, never conclusions
	 ...
	 */
	_buildOrchestratorInstruction(isFirst, { wrapUp = false } = {}) {
		...
		// Worker [DONE]s are completion signals, never conclusions — surface
		// them so the orchestrator (the sole concluding authority) decides.
		const signals =
			this._completionSignals.length > 0
				? `\nPeer completion signals (these peers reported their work complete — decide: emit action "done" to conclude, or route remaining work):\n` +
					this._completionSignals
						.map((s) => `  - ${s.peer} (round ${s.round})`)
						.join("\n") +
					"\n"
				: "";
...
The round limit (${this.maxRounds}) has been reached — this is your FINAL turn. You alone conclude the workflow: emit a single JSON object with action "done", summarizing what was accomplished, the key findings, and what remains open.

{"action": "done", "summary": "...", "final_artifacts": {}}

Do not route further work; conclude.`;
		}
...
final_artifacts values may be structured (objects/arrays) — the launcher renders them as human-readable Markdown in the conclusion.
...
You are the ONLY participant that may conclude the workflow. Peers always end with [YIELD] and return control to you — never wait for a peer to conclude; emit action "done" yourself when the work is complete.${signals}`;
		}
...
The last peer has completed their task. Decide the next action.${signals}

You are the ONLY participant that may conclude the workflow. Every peer report returns control to you — emit action "done" yourself when the work is complete; never wait for a peer to conclude.
...
Your response MUST be a single JSON object with the same schema as above.`;
	...
	_buildPeerInstruction(_peer, orchestratorInstruction, isFirst) {
		...
		if (orchestratorInstruction) {
			return (
				...
				`}\n\n` +
				`End with [YIELD] on its own line after your JSON report — [YIELD] hands control back to the Orchestrator. Never emit [DONE]: only the Orchestrator concludes the workflow. If you believe the work is complete, say so in your report and let the Orchestrator decide.`
			);
		}
	 ...
	 */
	async _speakOrchestrator({ wrapUp = false } = {}) {
	...
	async _speakPeer(peer, orchestratorInstruction) {
		...
		// Try to parse structured report (orchestrated mode)
		let structured = null;
		...
		// - Sequential: [DONE] from any peer concludes the discussion (legacy
		//   behavior, driven by the runtime instruction).
		// - Orchestrated: workers never conclude. Tokens inside a parsed JSON
		//   report are DATA (e.g. a codebase analysis mentioning "[DONE]"),
		//   not signals. [YIELD] outside the report hands control back to the
		//   orchestrator; [DONE] outside it is a completion SIGNAL for the
		//   orchestrator — only the orchestrator ever concludes.
		let yielded = false;
		let done = false;
		...
		if (this.mode === "orchestrated") {
			const span = structured ? findJSONSpan(finalText) : null;
			const outside = span
				? finalText.slice(0, span.start) + finalText.slice(span.end)
				: finalText;
			if (outside.includes(YIELD_TOKEN)) {
				yielded = true;
				display = stripOutsideSpan(display, span, YIELD_TOKEN);
			}
			if (outside.includes(DONE_TOKEN)) {
				completionSignal = true;
				display = stripOutsideSpan(display, span, DONE_TOKEN);
			}
		} else if (display.includes(DONE_TOKEN)) {
			done = true;
	...
	_recordConclusion({
		...
		// Split artifacts into flat strings (paths, names — shown in one line)
		// and structured values (objects/arrays — rendered as human-readable
		// Markdown docs). String() coercion of structures produced
		// "[object Object]" garbage; rendering them fixes that.
		const artifactSet = new Set();
		const artifactDocs = [];
		const addArtifact = (name, value) => {
			if (value === null || value === undefined) return;
			if (isStructuredArtifact(value)) {
				const md = artifactToMarkdown(name, value);
				if (md) artifactDocs.push({ name, markdown: md });
				return;
			}
			const s = String(value).trim();
			if (s !== "" && s !== "undefined" && s !== "null") artifactSet.add(s);
		};
		if (Array.isArray(finalArtifacts)) {
			finalArtifacts.forEach((a, i) => addArtifact(`artifact ${i + 1}`, a));
		} else {
			for (const [name, value] of Object.entries(finalArtifacts ?? {})) {
				addArtifact(name, value);
			}
		}
		if (orchestrated) {
			for (const [name, value] of Object.entries(this.workflowState.artifacts)) {
				addArtifact(name, value);
			}
			for (const finding of this._collectPeerFindings()) {
				for (const a of finding.artifacts ?? [])
					addArtifact(`${finding.peer} artifact`, a);
			}
		}
			...
			while (this.round < this.maxRounds && !consensus) {
				...
				if (this.workflowState.consecutive_routes >= 3) {
					...
					if (alternatives.length > 0) {
						...
						// A worker's [DONE] is a completion signal, never a conclusion.
						if (result.completionSignal) {
							this._recordCompletionSignal(altPeer.name);
						}
						continue;
					}
				}
				...
				// A worker's [DONE] is a completion signal, not a conclusion: the
				// orchestrator alone decides when the workflow ends.
				if (workerResult.completionSignal) {
					this._recordCompletionSignal(nextPeer.name);
				}
			}

			// Round-limit wrap-up: the orchestrator ALWAYS gets the last say. One
			// final turn to conclude (or report what remains) even when the
			// round limit was hit without a conclusion.
			if (!consensus && !brokeOnError && this.round >= this.maxRounds) {
				this.round++;
				this._log(
					this._color(
						"user",
						`\n${this._buildRoundHeader(this.round, { name: "orchestrator", role: "orchestrator" }, this.maxRounds, "wrap-up")}\n`,
					),
				);
				try {
					const wrapResult = await this._speakOrchestrator({ wrapUp: true });
					const wrapAction = parseAgentJSON(wrapResult.text);
					if (
						wrapAction &&
						wrapAction.action === "done" &&
						validateOrchestratorAction(wrapAction)
					) {
						this.transcript.push({
							peer: "orchestrator",
							role: "orchestrator",
							...
							round: this.round,
							structured: wrapAction,
						});
						this._displayOrchestratorDecision(wrapAction);
						consensus = true;
						this._recordConclusion({
							byPeer: "orchestrator",
							byRole: "orchestrator",
							round: this.round,
							summary: wrapAction.summary,
							structured: wrapAction,
							finalArtifacts: wrapAction.final_artifacts ?? null,
						});
					} else {
						this._log(
							this._color(
								"critic",
								"\n[wrap-up: orchestrator did not conclude — run ends without consensus]\n",
							),
						);
					}
				} catch (err) {
					this._log(this._color("critic", `\n[wrap-up error: ${err.message}]\n`));
				}
			}
		} finally {
			await this.shutdown();
	...
	_displayOrchestratorDecision(action) {
		if (this.pretty) {
			...
			this._log(this._color("orchestrator", `✅ Consensus Reached\n`));
			this._log(this._color("orchestrator", `   Summary: ${action.summary}\n`));
		} else if (action.action === "fallback") {
			this._log(this._color("orchestrator", `⚠️  Fallback to sequential\n`));
		}
	}
	 ...
	 */
	/**
	 * Record a worker [DONE] (orchestrated mode): a completion signal passed
	 * into the orchestrator's next context. Workers never conclude — the
	 * orchestrator alone decides when the workflow ends.
	 */
	_recordCompletionSignal(peerName) {
		this._completionSignals.push({ peer: peerName, round: this.round });
	}

	/**
	 * Run a single round-robin worker turn as an orchestrated-mode fallback
	 * (used when the orchestrator's response can't be parsed or routed).
	 * A worker's [DONE] here is a completion signal for the orchestrator,
	 * never a conclusion.
	 */
	async _fallbackTurn() {
		...
		if (result.completionSignal) {
			this._recordCompletionSignal(peer.name);
		}
	}
	...
	_buildConclusionBlock(consensus) {
		...
		// Structured artifacts, rendered as human-readable Markdown (objects/
		// arrays from final_artifacts and workflow state — not file paths).
		if (c.artifactDocs && c.artifactDocs.length > 0) {
			for (const doc of c.artifactDocs) {
				lines.push(this._color("bold", `Artifact: ${doc.name}`));
				lines.push("");
		...
		// Rendered BEFORE the summary so the orchestrator's conclusion is
		// always the last thing shown before the footer.
		...
		// The orchestrator's summary is always the last word.
